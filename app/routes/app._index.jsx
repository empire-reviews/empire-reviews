...
import { Page, Layout, BlockStack, InlineGrid, InlineStack, Text, } from "@shopify/polaris";
...
export const loader = async ({ request }) => {
    ...
    for (const key of ["shop", "host", "embedded", "locale", "session", "timestamp", "hmac"]) {
        if (url.searchParams.has(key))
            shopifyParams.set(key, url.searchParams.get(key));
    }
    ...
    if (planName === "EMPIRE_PRO") {
        ...
        if (sentimentScore >= 4.5)
            sentimentLabel = "Exceptional Love 🚀";
        else if (sentimentScore >= 4.0)
            sentimentLabel = "High Trust 💎";
        else if (sentimentScore >= 3.0)
            sentimentLabel = "Room to Improve";
        else if (totalReviews > 0)
            sentimentLabel = "Critical Action Needed";
        let insights = {
        ...
        if (settings?.aiProvider && settings?.aiApiKey) {
            ...
            if (needsRefresh && totalReviews > 0) {
                try {
                    ...
                    const aiResult = await generateInsights({ provider: settings.aiProvider, apiKey: settings.aiApiKey || "" }, recentReviews);
...
export default function EmpireDashboard() {
    const { metrics, planName, impact, insights } = useLoaderData();
    const navigate = useNavigate();
    return (<div className="empire-dashboard">
      {/* CUSTOM STYLES */}
      <style>{`
            .empire-dashboard {
            ...
            <div className="hero-content">
              <BlockStack gap="400">
                ...
                <InlineStack gap="300">
                  ...
                  {planName === "FREE" && (<button className="action-btn upgrade-pulse" onClick={() => navigate("/app/plans")} style={{ background: 'white', color: '#0f172a', border: '1px solid #e2e8f0' }}>
                      Upgrade to Pro 💎
                    </button>)}
                </InlineStack>
              </BlockStack>
            ...
            <Layout.Section>
              <InlineGrid columns={{ xs: 1, sm: 3 }} gap="400">
                ...
                <div className="stat-card">
                  ...
                  <div style={{ marginTop: '1rem' }}>
                    <div style={{ display: 'flex', gap: '2px' }}>
                      {[1, 2, 3, 4, 5].map(s => (<div key={s} style={{ flex: 1, height: '4px', background: s <= Math.round(metrics.averageRating) ? '#f59e0b' : '#e2e8f0', borderRadius: '2px' }}></div>))}
                    </div>
                  </div>
                </div>
                ...
                <div className="stat-card" style={{ borderColor: metrics.unrepliedCount > 0 ? '#fca5a5' : '#e2e8f0' }}>
                  ...
                  {metrics.unrepliedCount > 0 ? (<button onClick={() => navigate("/app/reviews")} style={{
                width: '100%', padding: '8px', marginTop: '1rem', background: '#fee2e2', color: '#b91c1c', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600
                      ...
                      Clear Queue →
                    </button>) : (<div style={{ marginTop: '1rem', color: '#10b981', fontWeight: 600 }}>All Caught Up! 🎉</div>)}
                </div>
              </InlineGrid>
            </Layout.Section>
          </Layout>
          ...
          <Layout>
            <Layout.Section>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                ...
                <div onClick={() => navigate("/app/impact")} style={{
            ...
            overflow: 'hidden',
                    ...
                    {planName === "EMPIRE_PRO" ? (<div>
                        ...
                        <div className="blurred-preview" style={{ marginTop: '6px' }}>
                          <div style={{ height: '28px', display: 'flex', alignItems: 'flex-end', gap: '3px' }}>
                            {[40, 70, 45, 90, 65, 80, 50].map((h, i) => (<div key={i} style={{ flex: 1, background: 'rgba(255,255,255,0.3)', height: `${h}%`, borderRadius: '2px 2px 0 0' }}></div>))}
                          </div>
                        </div>
                ...
                {/* AI INSIGHTS CARD */}
                <div onClick={() => navigate("/app/insights")} style={{
            ...
            overflow: 'hidden'
                  ...
                  <BlockStack gap="200">
                    ...
                    {planName === "EMPIRE_PRO" ? (<div>
                        ...
                        {insights?.aiSummary && (<div style={{ fontSize: '0.8rem', opacity: 0.9, marginTop: '6px', lineHeight: '1.4' }}>
                            {insights?.aiSummary}
                          </div>)}
            ...
            overflow: 'hidden'
                ...
                {/* IMPORT CARD */}
                <div onClick={() => navigate("/app/import")} style={{
            ...
            overflow: 'hidden'
                ...
                {/* CAMPAIGNS CARD */}
                <div onClick={() => navigate("/app/campaigns")} style={{
