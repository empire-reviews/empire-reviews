...
export default function ImpactPage() {
    const { locked, stats } = useLoaderData();
    ...
    if (locked) {
        return (<div style={{
                ...
                overflow: 'hidden'
                    ...
                    <div style={{ padding: '60px', display: 'flex', flexDirection: 'column', gap: '80px', height: '100%' }}>
                        <div style={{ display: 'flex', gap: '30px', justifyContent: 'center' }}>
                            {[1, 2, 3, 4, 5].map(i => (<div key={i} style={{ width: '150px', height: '100px', background: 'rgba(16, 185, 129, 0.15)', borderRadius: '24px', border: '1px solid rgba(16, 185, 129, 0.3)' }}></div>))}
                        </div>

                        <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', gap: '15px', padding: '0 60px' }}>
                            {/* ROI Bar Growth Tease */}
                            {[30, 50, 40, 70, 55, 90, 65, 110, 80, 130, 95, 150].map((h, i) => (<div key={i} style={{
                    flex: 1,
                    height: `${h}px`,
                ...
                gap: '10px'
                                ...
                                <button onClick={() => navigate("/app")} style={{ background: 'transparent', border: 'none', color: '#64748b', cursor: 'pointer', fontWeight: 600, fontSize: '0.95rem' }}>
                                    Return to Dashboard
                                </button>
                            </BlockStack>
                        </BlockStack>
