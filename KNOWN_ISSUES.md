...
- [x] **C8. "50-review free cap" never enforced.** Advertised in plans/settings/dashboard/onboarding ("prevent data loss") but `api.reviews.tsx:139` create has no count/plan check; import allows 500 rows for free users. **Fix:** enforce server-side or remove the claims (dark-pattern copy).
