...
import { BlockStack, Text, Button, DropZone, InlineStack, Badge, } from "@shopify/polaris";
...
function parseCSV(text) {
    ...
    for (let i = 1; i < arr.length; i++) {
        ...
        if (rowData.length < 2 && (!rowData[0] || rowData[0].trim() === ""))
            continue;
        const obj = {};
        headers.forEach((header, index) => {
            ...
            if (['rating', 'stars', 'star'].includes(header))
                obj['rating'] = value;
            ...
            else if (['name', 'author', 'customer', 'reviewer', 'reviewername', 'customername'].includes(header))
                obj['customer'] = value;
            else if (['email', 'revieweremail', 'customeremail'].includes(header))
                obj['email'] = value;
            else if (['title', 'reviewtitle', 'headline'].includes(header))
                obj['review_title'] = value;
            else if (['date', 'createdat', 'reviewdate', 'timestamp', 'reviewdate'].includes(header))
                obj['date'] = value;
            else if (['reply', 'response', 'ownerreply'].includes(header))
                obj['reply'] = value;
            else if (['picture_urls', 'pictureurls', 'images', 'photos', 'media'].includes(header))
                obj['images'] = value;
            else if (['product_id', 'productid', 'id', 'productname'].includes(header))
...
async function resolveProductsSmartly(admin, identifiers) {
    ...
    if (handles.length > 0) {
        ...
        try {
            ...
            data?.products?.nodes?.forEach((p) => productMap.set(p.handle, p.id));
        }
        catch (e) {
            console.error(e);
        }
    ...
    if (titles.length > 0) {
        ...
        try {
            ...
            const { data } = await response.json();
            data?.products?.nodes?.forEach((p) => productMap.set(p.title, p.id));
        }
        catch (e) {
            console.error(e);
        }
...
export const action = async ({ request }) => {
    ...
    try {
        ...
        if (!file.name.endsWith('.csv'))
            return json({ success: false, message: "Invalid file type. Please upload a .csv file." });
        const text = await file.text();
        const records = parseCSV(text);
        const allIdentifiers = records.map((r) => ({ handle: r.handle, title: r.product_title }));
        ...
        for (const record of records) {
            ...
            if (!productId && record.handle)
                productId = productMap.get(record.handle);
            if (!productId && record.product_title)
                productId = productMap.get(record.product_title);
        ...
        for (const review of complexReviews) {
            const mediaCreate = [];
            if (review.images) {
                const urls = review.images.split(',').map((u) => u.trim());
            ...
            const repliesCreate = [];
            if (review.reply)
                repliesCreate.push({ body: review.reply });
            // Remove helper props
...
export default function ImportPage() {
    ...
    const handleDrop = useCallback(async (_droppedFiles, acceptedFiles, _rejectedFiles) => {
        ...
        const platforms = [];
        if (text.includes('judgeme'))
            platforms.push('Judge.me');
        if (text.includes('yotpo'))
            platforms.push('Yotpo');
        if (text.includes('loox'))
            platforms.push('Loox');
    ...
    return (<div className="empire-import">
            <style>{`
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 2rem 4rem 2rem' }}>
            ...
            ].map((trust, i) => (<div key={i} className="trust-badge-3d">
                ...
                {step === 2 && previewData && (<div className="tilt-card" style={{
                background: 'white',
                borderRadius: '40px',
                padding: '3rem',
                border: '1px solid #e2e8f0',
                ...
                margin: '0 auto'
                        ...
                        <BlockStack gap="600">
                            ...
                            <div style={{ background: previewData.hasBody ? '#ecfdf5' : '#fef2f2', padding: '1.5rem', borderRadius: '20px', border: `1px solid ${previewData.hasBody ? '#10b981' : '#ef4444'}` }}>
                                <BlockStack gap="200">
                                    <InlineStack align="space-between">
                                        ...
                                        {previewData.hasBody ? (<Badge tone="success">✅ Body Found</Badge>) : (<Badge tone="critical">❌ BODY MISSING</Badge>)}
                                    </InlineStack>
                                    {!previewData.hasBody && (<Text as="p" tone="critical">
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                                    <thead>
                                        <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                                            ...
                                            {previewData.headers.map((h, i) => (<th key={i} style={{ padding: '12px', textAlign: 'left', textTransform: 'capitalize' }}>{h}</th>))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {previewData.samples.map((row, i) => (<tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                                ...
                                                {previewData.rawSamples[i].map((cell, ci) => (<td key={ci} style={{ padding: '12px', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                        {cell}
                                                    </td>))}
                                            </tr>))}
                                    </tbody>
                            ...
                            <BlockStack gap="400">
                                <Button size="large" variant="primary" tone="success" onClick={handleImport} loading={fetcher.state === "submitting"} disabled={!previewData.hasBody} fullWidth>
                ...
                {(step === 2 || step === 3) && (<div className="hero-card">
                        <BlockStack gap="600">
                            {/* STEP 2: AUDIT */}
                            {step === 2 && auditData && (<BlockStack gap="600">
                                    <div style={{ textAlign: 'center' }}>
                                    ...
                                    <InlineStack align="center" gap="400">
                                        ...
                                        <div style={{ minWidth: '200px' }}>
                                            <Button size="large" variant="primary" tone="success" onClick={handleImport} loading={fetcher.state === "submitting"} fullWidth>
                                                Launch Migration →
                                            </Button>
                                        </div>
                                    </InlineStack>
                            ...
                            {/* STEP 3: SUCCESS */}
                            {step === 3 && (<BlockStack gap="600" align="center">
                                    <div style={{
                    width: '100px',
                    height: '100px',
                    background: '#10b981',
