 ...
 */
export function getConversionPhase(appInstalledAt) {
    const daysSinceInstall = Math.floor((Date.now() - appInstalledAt.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
export function shouldShowUpgradePrompt(phase, lastUpgradePrompt) {
    // Never show in DELIGHT phase (first 7 days)
    if (phase === "DELIGHT")
        return false;
    // If never shown before, OK to show
    if (!lastUpgradePrompt)
        return true;
    // Check if 7 days have passed since last prompt
    const daysSinceLastPrompt = Math.floor((Date.now() - lastUpgradePrompt.getTime()) / (1000 * 60 * 60 * 24));
    return daysSinceLastPrompt >= 7;
}
/**
 * Record that we showed an upgrade prompt
 */
export async function recordUpgradePrompt(shop) {
    try {
