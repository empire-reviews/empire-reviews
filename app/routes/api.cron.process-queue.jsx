...
export const loader = async ({ request }) => {
    ...
    for (const order of pendingOrders) {
            ...
            try {
                ...
                if (shopifyOrder?.fulfillments && shopifyOrder.fulfillments.length > 0) {
                    ...
                    for (const f of shopifyOrder.fulfillments) {
                        if (f.events?.nodes) {
                            const deliveredEvent = f.events.nodes.find((e) => e.status === 'DELIVERED');
        ...
        const emailResult = await sendReviewRequest(order.customerEmail, "Customer", "your recent order", reviewLink, order.shop);
