...
export const sendReviewRequest = async (toEmail, customerName, productTitle, reviewLink, shopDomain) => {
...
export const sendCampaignEmail = async (shopDomain, toEmail, subject, bodyHtml, trackingId) => {
    // 1. Check if user is unsubscribed
    const isUnsubscribed = await prisma.unsubscriber.findUnique({
        where: { email_shop: { email: toEmail, shop: shopDomain } }
    });
