...
import { Page, Layout, Card, BlockStack, Text, Checkbox, Button, TextField, Divider, Modal, Badge, Select, ActionList, InlineGrid } from "@shopify/polaris";
import { authenticate } from "../shopify.server";
import { hasActivePayment, getPlanDetails } from "../billing.server";
import prisma from "../db.server";
import { useState, useEffect } from "react";
import { testAIConnection } from "../services/ai.server";
...
export const action = async ({ request }) => {
    ...
    if (intent === "test_ai") {
        ...
        const aiApiKey = formData.get("aiApiKey");
    ...
    // AI Configuration
    let aiProvider = formData.get("aiProvider") || null;
    let aiApiKey = formData.get("aiApiKey") || null;
    // Integrations
    ...
    const reviewRequestDelay = parseInt(formData.get("reviewRequestDelay")) || 3;
...
export default function SettingsPage() {
    const { settings, isPro, subscription } = useLoaderData();
    ...
    return (<Page title="Settings" subtitle="Manage your Empire Reviews configuration" backAction={{ content: 'Dashboard', onAction: () => navigate("/app") }} primaryAction={isDirty ? { content: 'Save settings', onAction: handleSave } : undefined} fullWidth>
            <style>{`
                .stat-card {
                    background: white;
                    padding: 1.5rem;
            ...
            <Layout>
                <Layout.Section variant="oneThird">
                    <BlockStack gap="400">
                        <Card padding="0">
                            <ActionList actionRole="menuitem" items={tabs.map(tab => ({
            content: tab.content,
            icon: tab.icon,
            ...
            onAction: () => setActiveTab(tab.id)
                ...
                <Layout.Section>
                    {activeTab === 'brand' && (<BlockStack gap="400">
                    ...
                    {activeTab === 'automation' && (<BlockStack gap="400">
                    ...
                    {activeTab === 'ecosystem' && (<BlockStack gap="400">
                                ...
                                <div style={{ padding: '0 20px 20px' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                                        <div style={{ flex: 1, paddingRight: '24px' }}>
                                            ...
                                            {klaviyoEnabled && (<TextField labelHidden label="API Key" value={klaviyoKey} onChange={setKlaviyoKey} autoComplete="off" type="password" placeholder="pk_..."/>)}
                                        </div>
                                        <div style={{ flexShrink: 0 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                                        <div style={{ flex: 1, paddingRight: '24px' }}>
                                            ...
                                            {googleShoppingEnabled && isPro && (<div style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', fontSize: '0.85rem', border: '1px solid #e2e8f0' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                                        <div style={{ flex: 1, paddingRight: '24px' }}>
                                            ...
                                            {isPro && (<BlockStack gap="300">
                                                    ...
                                                    {aiProvider && (<TextField labelHidden label={aiProvider === 'ollama' ? "Model Name / Remote URL / API Key" : "Secret API Key"} value={aiApiKey} onChange={setAiApiKey} autoComplete="off" type={aiProvider === 'ollama' ? "text" : "password"} placeholder={aiProvider === 'ollama' ? "API Details" : "Secret Key"} helpText={aiProvider === 'openai' ? 'Get yours at platform.openai.com' : aiProvider === 'gemini' ? 'Get yours at aistudio.google.com' : aiProvider === 'claude' ? 'Get yours at console.anthropic.com' : aiProvider === 'deepseek' ? 'Get yours at platform.deepseek.com' : aiProvider === 'ollama' ? 'Format: URL|Model|API_KEY (e.g. https://ollama.com|gpt-oss:120b|sk-123). URL and Key are optional.' : ''}/>)}
                                                    {aiProvider && (<div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                            <Button onClick={() => { setAiTestLoading(true); setAiTestResult(null); fetcher.submit({ intent: 'test_ai', aiProvider, aiApiKey }, { method: 'post' }); setTimeout(() => { setAiTestLoading(false); const data = fetcher.data; if (data?.aiTestResult) {
                ...
                } }, 4000); }} loading={aiTestLoading} disabled={aiTestLoading || !aiApiKey} size="micro">Test Connection</Button>
                    ...
                    {activeTab === 'billing' && (<BlockStack gap="400">
                            ...
                            <Card padding="0">
                                ...
                                <div style={{ padding: '0 20px 20px' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        <div style={{ flexShrink: 0 }}>
                                            {isPro ? (<Button onClick={() => setBillingModalActive(true)}>Manage Plan</Button>) : (<Button variant="primary" onClick={handleUpgrade}>Upgrade Now</Button>)}
                                        </div>
                                    </div>
                                </div>
                            </Card>
                    ...
                    {activeTab === 'danger' && (<BlockStack gap="400">
            ...
            <Modal open={billingModalActive} onClose={() => setBillingModalActive(false)} title="Your Empire Membership" primaryAction={{ content: "Extend / Renew Plan", onAction: handleUpgrade, disabled: !isPro }} secondaryActions={[{ content: "Close", onAction: () => setBillingModalActive(false) }]}>
                <Modal.Section>
                    <BlockStack gap="400">
                        {subscription && subscription.status === "ACTIVE" ? (<BlockStack gap="200">
                                <Badge tone="success">Active Subscription</Badge>
                                <Text as="p" fontWeight="bold">Empire Pro App</Text>
                            </BlockStack>) : (<Text as="p">You are currently on the free Starter plan.</Text>)}
                        <Divider />
